...
public class HomeSteps extends CommonUtility{
    ...
    public void userCanSeeBelowOptionsArePresentInDepartment(io.cucumber.datatable.DataTable dataTable) {
        ...
        for (Map<String, String> row : data) {
            Assert.assertEquals(factory.homePage(), row.get("department"));
            Assert.assertEquals(factory.homePage(), row.get("optionOne"));
            Assert.assertEquals(factory.homePage(), row.get("optionTwo"));
